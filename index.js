...
const streamRouter = require("./router/stream");
...
app.use(compose([vastRouter.getRouter(), trackingRouter.getRouter(), streamRouter.getRouter()]));
...
Object.keys(config).map((scheme) => {
    console.log(`${scheme} port: ${config[scheme].port}`);
});
